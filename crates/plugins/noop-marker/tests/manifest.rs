//! Task 4.7: the shipped `manifest.yaml` loads via `faultforge-fault` and matches
//! the declared contract (identity, entrypoint, params schema, precondition,
//! duration limit).
...
fn shipped_manifest_conforms_to_contract() {
    ...
    assert_eq!(manifest.name.as_str(), "noop-marker");
    assert_eq!(manifest.version, "1");
    assert_eq!(manifest.identity(), "noop-marker@1");
    assert_eq!(manifest.entrypoint, "./noop-marker");

    assert_eq!(manifest.params_schema.len(), 1);
    assert_eq!(
        manifest.params_schema.get("marker_path"),
        Some(&ParamType::String)
    );

    assert!(manifest.requires.binaries.is_empty());
    assert!(manifest.requires.privileges.is_empty());
    assert_eq!(manifest.requires.preconditions, vec!["marker_dir_writable"]);

    assert_eq!(manifest.max_duration_secs, 600);
}
...
/// built binary equals what an operator gets from `cat manifest.yaml noop-marker
...
fn shipped_plugin_digest_reproduces_with_coreutils() {
    ...
    assert_eq!(
        ...
        coreutils_digest,
        "load_plugin digest must reproduce `cat manifest.yaml noop-marker | sha256sum`"
    );
}
