...
Defines the operator command-line tool (`faultforge`) for inspecting the
FaultForge master. The CLI supports one-shot scripting subcommands (`agents
list`, `agents show`) that talk to the master's HTTP management API, plus an
interactive terminal UI (TUI) launched when no subcommand is given on an
interactive terminal. The tool is configured by a single `--master-url` flag.
