...
Defines `noop-marker`, the reference plugin that demonstrates the fault plugin model contract.
It is a zero-blast-radius fault that injects a marker file and recovers by removing it, proving
the complete lifecycle without touching host services or processes. Serves as the golden reference
for plugin implementers and the contract-verification test for the shared fault crate.
